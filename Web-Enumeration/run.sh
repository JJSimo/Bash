...
if [ $# -lt 1 ];then
    ...
    if [ ! -d "$url/recon" ];then                                   # if the directory recon does not exist => create it
        sudo mkdir $url/recon
    fi
    if [ ! -d "$url/recon/scans" ];then
        sudo mkdir $url/recon/scans
    fi
    if [ ! -d "$url/recon/httprobe" ];then
        sudo mkdir $url/recon/httprobe
    fi
    if [ ! -d "$url/recon/potential_takeovers" ];then
        sudo mkdir $url/recon/potential_takeovers
    fi
    if [ ! -d "$url/recon/wayback" ];then
        sudo mkdir $url/recon/wayback
    fi
    if [ ! -d "$url/recon/wayback/params" ];then
        sudo mkdir $url/recon/wayback/params
    fi
    if [ ! -d "$url/recon/wayback/extensions" ];then
        sudo mkdir $url/recon/wayback/extensions
    fi
    if [ ! -f "$url/recon/httprobe/alive.txt" ];then
        sudo touch $url/recon/httprobe/alive.txt
    fi
    if [ ! -f "$url/recon/final.txt" ];then
        sudo touch $url/recon/final.txt
    fi
    ...
    cat $url/recon/final.txt | sort -u | /home/simone/go/bin/httprobe -s -p https:443 | sed 's/https\?:\/\///' | tr -d ':443' >> $url/recon/httprobe/a.txt              # check if subdomains are responding
